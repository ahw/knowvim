var ObjectMotions = {
    ...
    forwardParagraph : function(args) {
        ...
        while (!foundNextPosition) {
            ...
            if (row > lines.length - 1) {
                this.logger.debug('Hit last line of file. Putting cursor at end of last line');
                row--;
    ...
    backwardParagraph : function(args) {
        ...
        while (!foundNextPosition) {
            row--;
            if (row < 0) {
                this.logger.debug('Hit first line of file. Putting cursor at beginning of line');
                row++;
                col = 0;
                break;
            } else if (lines[row] == "") {
